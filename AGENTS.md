...
- **无构建系统**: 纯文档/代码学习仓库，无需npm/pip包管理器
- **无CI/CD**: 通过PR模板人工审查，无自动化测试流
- **本地测试**: Python文件包含 `if __name__ == "__main__":` 块用于快速测试
- **Mermaid绘图**: 分析文档广泛支持Mermaid图表（避免 `"` 和 `()` 出现在 `[]` 节点中）
- **虚拟头节点**: 链表操作时始终考虑使用虚拟头节点简化边界处理
- **稳定划分**: 链表/数组划分问题注意是否要求保持元素相对顺序
